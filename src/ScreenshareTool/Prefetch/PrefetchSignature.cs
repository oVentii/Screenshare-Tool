...
internal static class PrefetchSignature
{
    ...
    {
        // Fail closed: any unexpected native failure marks the file unsigned
        // (the safe direction for a screenshare review) instead of aborting.
        try
        {
            return VerifySignatureCore(filePath);
        }
        catch (Exception ex)
        {
    ...
    private static bool VerifySignatureCore(string filePath)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                if (status == 0)
                {
                    if (!CheckSignerChain(data.StateData))
                        valid = false;
                }
                else
                {
                    valid = false;
                    if (VerifyFileViaCatalog(filePath))
                        valid = true;
                }
    ...
    private static bool CheckSignerChain(IntPtr stateData)
    {
        try
        {
            ...
            if (certInfo == IntPtr.Zero)
                return true;
            int subjectOffset = IntPtr.Size == 8 ? 80 : 56;
            IntPtr subject = IntPtr.Add(certInfo, subjectOffset);

            byte[] nameBuf = new byte[256];
            int len = PrefetchNative.CertNameToStr(
                encoding, subject,
                PrefetchNative.CERT_X500_NAME_STR, nameBuf, (uint)nameBuf.Length);
            if (len > 1)
            {
                string subjectName = Encoding.ASCII.GetString(nameBuf, 0, len - 1).ToLowerInvariant();
                foreach (string cheat in CheatSubjects)
                {
                    if (subjectName.Contains(cheat, StringComparison.Ordinal))
                        return false;
                }
            }

            return !IsCertHashInSystemStores(certContext);
        }
        catch
        {
            return true;
    ...
    private static bool IsCertHashInSystemStores(IntPtr certContext)
    {
        try
        {
            ...
            try
            {
                ...
                // Certificates repeat across files (same Microsoft signer on
                // hundreds of binaries) — sweep the stores once per cert.
    ...
    private static bool VerifyFileViaCatalog(string filePath)
    {
        ...
        try
        {
            IntPtr file = PrefetchNative.CreateFile(
                filePath, PrefetchNative.GENERIC_READ, PrefetchNative.FILE_SHARE_READ,
            ...
            {
                uint hashSize = 0;
                if (!PrefetchNative.CatAdminCalcHashFromFileHandle(file, ref hashSize, IntPtr.Zero, 0))
                    return false;
                ...
                try
                {
                    ...
                    while (catalog != IntPtr.Zero)
                    {
                        ...
                        if (PrefetchNative.CatCatalogInfoFromContext(catalog, ref catalogInfo, 0))
                            info.CatalogFile = catalogInfo.CatalogFile ?? "";

                        bool verified = VerifyCatalogMember(
                            info.CatalogFile, filePath, hash, catalog, ref signed);
                        if (verified)
                            break;

                        catalog = PrefetchNative.CatAdminEnumCatalogFromHash(
    ...
    private sealed class CatalogInfoShim
    {
