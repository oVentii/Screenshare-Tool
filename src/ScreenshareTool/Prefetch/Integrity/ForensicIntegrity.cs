...
internal static class ForensicIntegrity
{
    ...
    private static readonly System.Collections.Concurrent.ConcurrentDictionary<string, List<EventClear>> ClearCache = new();
    public static List<EventClear> DetectEventLogClears(DateTime bootLocal)
    {
        string key = bootLocal.ToString("O");
        if (ClearCache.TryGetValue(key, out var cached)) return cached;
    ...
    private static void TryAdd(List<EventClear> hits, string label, string logName, int eventId, DateTime bootLocal)
    {
    ...
    public static string BuildEventFilter(int eventId, DateTime? after)
    {
        if (!after.HasValue)
            return "*[System[(EventID=" + eventId + ")]]";

        string iso = after.Value.ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss.fffZ", CultureInfo.InvariantCulture);
