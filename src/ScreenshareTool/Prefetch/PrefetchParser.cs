...
internal sealed class PrefetchParser
{
    ...
    public long[] LastEightExecutionTimes()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            try
            {
                times[i] = DateTime.FromFileTimeUtc(ft).ToLocalTime().Ticks / TimeSpan.TicksPerSecond
    ...
    public long ExecutedTime()
    {
        ...
        try
        {
            return DateTime.FromFileTimeUtc(ft).ToLocalTime().Ticks / TimeSpan.TicksPerSecond
                - DateTime.UnixEpoch.Ticks / TimeSpan.TicksPerSecond;
        }
        catch
        {
