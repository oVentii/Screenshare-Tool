...
internal static class PrefetchNative
{
    ...
    // SECURITY_LOGON_SESSION_DATA field offsets (x64):
    // Size@0, LogonId@8, UserName@16, LogonDomain@32,
    // AuthenticationPackage@48, LogonType@64, Session@68, Sid@72, LogonTime@80.
    ...
    {
        public uint StructSize;
        // Native CATALOG_INFO embeds WCHAR wszCatalogFile[MAX_PATH] inline,
        // so this must be a fixed buffer, not a string pointer.
    ...
    [DllImport("crypt32.dll", CharSet = CharSet.Ansi, SetLastError = true, EntryPoint = "CertNameToStrA")]
    ...
    // NOTE: the CryptCAT* catalog APIs are declared in mscat.h but
    // implemented in wintrust.dll — there is no mscat.dll on Windows.
