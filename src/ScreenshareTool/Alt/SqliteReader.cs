...
internal static class SqliteReader
{
    ...
    private static void ReadTablePage(byte[] file, int page, int pageSize, Action<List<string>> onRow, HashSet<int> visited, int depth)
    {
        if (depth > 32 || !visited.Add(page)) return;
        ...
        else if (type == 5)
        {
            ...
            if (rightChild > 1) ReadTablePage(file, rightChild, pageSize, onRow, visited, depth + 1);
            for (int i = 0; i < cellCount; i++)
            {
                ...
                if (child > 1) ReadTablePage(file, child, pageSize, onRow, visited, depth + 1);
            }
        }
    }
