...
public sealed class AltDetectorScanner
{
    ...
    private static readonly Regex PlayerName = new(@"^Player\d+$", RegexOptions.IgnoreCase | RegexOptions.Compiled);
    private static readonly Regex DiscordIdCore = new(@"(\d{17,19})", RegexOptions.Compiled);
    private static readonly Regex DiscordIdJson = new("\"id\"\\s*:\\s*\"\\d{17,19}\"", RegexOptions.Compiled);
    private static readonly Regex DiscordNameJson = new("\"username\"\\s*:\\s*\"([^\"]{2,32})\"", RegexOptions.Compiled);
    ...
    public async Task<AltDetectorResult> RunAsync(IProgress<string>? progress = null, CancellationToken ct = default)
    {
        ...
        await ScanLogFilesAsync(ct).ConfigureAwait(false);
    ...
    private void LoadCachedResults(CancellationToken ct)
    {
        ...
        try
        {
            foreach (var line in File.ReadAllLines(CacheFile))
            {
                ...
                if (kind == "MC" && IsValidUsername(value) && _users.Add(value))
                    _cachedMcCount++;
                else if (kind == "DC" && IsValidDiscordId(value) && _discordIds.Add(value))
                    _cachedDcCount++;
            }
        }
        catch { }
    ...
    private List<string>? _userDirs;

    private IEnumerable<string> GetUserDirs() => _userDirs ??= CollectFixedUserDirs();

    private static List<string> CollectFixedUserDirs()
    {
    ...
    {
        foreach (var userDir in GetUserDirs())
        {
            ct.ThrowIfCancellationRequested();
        ...
        try
        {
            ...
            if (string.IsNullOrEmpty(pf86))
                pf86 = Environment.GetEnvironmentVariable("ProgramFiles(x86)") ?? @"C:\Program Files (x86)";
            ProcessLauncherFile(Path.Combine(pf86, @"Minecraft\launcher_profiles.json"));
        }
        catch { }
    }
    ...
    private static bool IsValidUsername(string username)
    {
        ...
        if (PlayerName.IsMatch(username)) return false;
        return true;
    }
    ...
    {
        int run = 0;
        int limit = Math.Min(bytes.Length, 10 * 1024 * 1024);
    ...
    {
        Match idMatch = DiscordIdCore.Match(key);
        if (!idMatch.Success) return;
        string id = idMatch.Groups[1].Value;
        if (!IsValidDiscordId(id)) return;
        if (DiscordIdJson.IsMatch(value)) return;

        Match nameMatch = DiscordNameJson.Match(value);
    ...
    private static bool IsKnownMcDotDir(string name)
    {
        return name.Equals(".minecraft", StringComparison.OrdinalIgnoreCase)
            || name.Equals(".lunarclient", StringComparison.OrdinalIgnoreCase)
            || name.Equals(".feather", StringComparison.OrdinalIgnoreCase)
            || name.Equals(".tlauncher", StringComparison.OrdinalIgnoreCase)
            || name.Equals(".meteor", StringComparison.OrdinalIgnoreCase)
            || name.Equals(".sigma", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private async Task ScanLogFilesAsync(CancellationToken ct)
    {
        ...
        foreach (var userDir in GetUserDirs())
        {
            ...
            CollectMcLogs(Path.Combine(userDir, @"AppData\Roaming\.minecraft\logs"), files, ct);
    ...
    private static void CollectMcLogs(string dir, List<string> files, CancellationToken ct)
    {
        string[] found;
        try
        {
            if (!Directory.Exists(dir)) return;
            found = Directory.GetFiles(dir, "*.log*", SearchOption.TopDirectoryOnly);
        }
        catch { return; }
        foreach (var f in found)
        {
            ...
            try { if (new FileInfo(f).Length <= 1024 * 1024) files.Add(f); }
            catch { }
        }
    }
    ...
    private void ScanDirectoryRecursive(string directory, List<string> fileList, CancellationToken ct, int depth = 0)
    {
        ...
        foreach (var d in subdirs)
        {
            ...
            if (name.StartsWith('.') && !IsKnownMcDotDir(name)) continue;
