...
internal static class EmbeddedAssetExtractor
{
    ...
    private static string Extract()
    {
        ...
        using var mutex = new System.Threading.Mutex(false, @"Global\iRis-Screenshare-Tool-Embedded");
        bool owned = false;
        try { owned = mutex.WaitOne(TimeSpan.FromSeconds(30)); }
        catch { owned = false; }
        try
        {
            return ExtractCore(root);
        }
        finally
        {
            if (owned)
    ...
    private static string ExtractCore(string root)
    {
        try
        {
        ...
        if (File.Exists(Path.Combine(root, "Assets", "index.html")) &&
            File.Exists(markerPath) && MarkerMatches(markerPath, version))
        {
            return root;
        }
        TryDeleteTree(root);
    ...
    private static bool MarkerMatches(string markerPath, string version)
    {
        try
        {
            return File.ReadAllText(markerPath) == version;
        }
        catch
        {
            return false;
