...
internal static class CheatRules
{
    ...
    private static void EvalF5(Ctx ctx, List<RuleHit> hits)
    {
        if (!ctx.Pe.HasDotNet || !SizeOk(ctx, strict: false))
            return;
        int textIdx = ctx.Pe.SectionIndex(".text");
        if (textIdx < 0 || textIdx >= ctx.Pe.Sections.Count)
            return;
        var sec = ctx.Pe.Sections[textIdx];
        if (sec.RawSize == 0)
            return;
        for (int i = 0; i < ctx.Pe.DotNetStreams.Count; i++)
        {
            ...
            if (st.Size >= 1000)
                continue;
            uint probe = Math.Min(st.Size, sec.RawSize);
            double? ent = ctx.Pe.Entropy(sec.RawOff, probe);
    ...
    private static double? StreamEntropy(string path)
    {
        try
        {
            ...
            long total = 0;
            const long entropyCap = 512L * 1024 * 1024;
            byte[] chunk = new byte[1048576];
            int n;
            while (total < entropyCap && (n = fs.Read(chunk, 0, chunk.Length)) > 0)
            {
                total += n;
                for (int i = 0; i < n; i++)
                    counts[chunk[i]]++;
