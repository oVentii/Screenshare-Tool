...
public static class ServiceChecker
{
    ...
    public static ServiceCheckResult Run(CancellationToken ct = default)
    {
        try
        {
        ...
        {
            return new ServiceCheckResult { Error = "Scan cancelled." };
        }
        ...
        {
            Logger.Error(ex, "Service checker failed");
            return new ServiceCheckResult { Error = "Check failed: " + ex.Message };
        }
    }
    ...
    private static List<ServiceEntry> CollectServices(CancellationToken ct)
    {
        ...
        foreach (var (names, fallbackDisplay, critical) in WatchedServices)
        {
            ...
            if (!TryOpenService(names, out ServiceController? service, out string resolvedName) ||
                service is null)
            {
    ...
    private static bool TryOpenService(string[] names, out ServiceController? service, out string resolvedName)
    {
    ...
    private static void ApplyScmTimes(
        List<ServiceEntry> missing, int eventId, bool runningStatesOnly, CancellationToken ct)
    {
        try
        {
            var q = new EventLogQuery("System", PathType.LogName, "*[System[(EventID=" + eventId + ")]]")
            {
            ...
            while (remaining > 0 && scanned++ < ScmEventScanLimit)
            {
                ...
                if (runningStatesOnly)
                {
                    if (IsStoppedState(extra))
                        continue;
                }
                else if (IsStopControl(extra))
                {
                    continue;
                }
        ...
        {
            Logger.Debug(ex, "SCM event {EventId} lookup failed", eventId);
        }
    }
    ...
    private static bool IsStopControl(string? control)
    {
        if (string.IsNullOrEmpty(control)) return false;
        if (control.IndexOf("start", StringComparison.OrdinalIgnoreCase) >= 0)
            return false;
        return control.IndexOf("stop", StringComparison.OrdinalIgnoreCase) >= 0
            || control.IndexOf("paus", StringComparison.OrdinalIgnoreCase) >= 0
            || control.IndexOf("continu", StringComparison.OrdinalIgnoreCase) >= 0;
    }
