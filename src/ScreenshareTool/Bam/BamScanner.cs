...
public static class BamScanner
{
        ...
        CancellationToken ct)
    {
        ...
        foreach (string sub in key.GetSubKeyNames())
        {
            ...
            {
                ct.ThrowIfCancellationRequested();
                if (subKey.GetValueKind(valueName) != RegistryValueKind.Binary)
                    continue;
                ...
                if (ft != 0)
                {
                    try
                    {
                        DateTime local = DateTime.FromFileTimeUtc(ft).ToLocalTime();
                        // True UTC unix for sorting/relative math; the
                        // display string stays local.
                        unix = DateTime.FromFileTimeUtc(ft).Ticks / TimeSpan.TicksPerSecond
                ...
                rows.Add((ResolveDevicePath(valueName), unix, readable));
            }
        }
        return rows;
    }

    internal static string ResolveDevicePath(string path)
    {
        ...
        try
        {
            ...
            foreach (var drive in DriveInfo.GetDrives())
            {
                try
                {
                    ...
                    // Boundary-checked: "\Device\HarddiskVolume4" must not
                    // match "\Device\HarddiskVolume44\...".
        ...
        // Keep the raw device path for display instead of a mangled "?:" form;
        // existence checks fail on it either way.
        return path;
    }
        ...
        string readableTime, List<long> logons, long nowUnix)
    {
        ...
        return execUnix >= logons[0] && execUnix <= nowUnix;
    }
}
